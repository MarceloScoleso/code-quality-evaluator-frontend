        ...
        <div className="max-w-2xl mx-auto px-6 space-y-8">
          ...
          {user && (
            <div className="rounded-2xl border border-slate-800 bg-slate-900/50 overflow-hidden">
              ...
              <div className="flex items-center gap-3 px-5 py-4 bg-white/[0.02] border-b border-slate-800">
                ...
                <span className="text-[0.78rem] font-semibold text-slate-300">{t("account.personalData")}</span>
              </div>
 
              <div className="p-5 space-y-3">
                {[
                  { icon: <UserCircle size={14} />,  label: t("account.name"),      value: user.name },
                  { icon: <Mail size={14} />,         label: t("account.email"),     value: user.email },
                  { icon: <CalendarDays size={14} />, label: t("account.createdAt"), value: new Date(user.createdAt).toLocaleDateString() },
                  { icon: <ShieldCheck size={14} />,  label: t("account.internalId"), value: `#${user.id}` },
                ].map((row) => (
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/40 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <p className="text-sm font-semibold text-white mb-1">{t("account.exportTitle")}</p>
              ...
              {(deleteStep === "confirm" || deleteStep === "deleting") && (
                <div className="mt-4 pt-4 border-t border-red-500/15 space-y-3">
                  <p className="text-[0.8rem] text-slate-300 font-medium">
                    {t("account.deleteConfirmTitle")} {t("account.deleteConfirmDesc")}
                  </p>
