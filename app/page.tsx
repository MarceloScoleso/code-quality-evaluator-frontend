...
const steps = [
  ...
  },
  {
    ...
    description:
      "O sistema processa as métricas e gera um score técnico com classificação baseada em critérios estruturais e boas práticas.",
  },
  {
    number: "03",
    icon: "📈",
...
const stats = [
  { val: "4 níveis",   label: "de classificação" },
  { val: "100%",       label: "automatizado"      },
  { val: "N métricas", label: "por projeto"       },
  { val: "Dashboard",  label: "em tempo real"     },
];
 
const metrics = [
  { name: "Cobertura de testes",     pct: 82  },
  { name: "Complexidade estrutural", pct: 91  },
  { name: "Controle de versão",      pct: 100 },
  { name: "Boas práticas",           pct: 74  },
];
...
export default function LandingPage() {
  return (
    <div className="font-sans bg-slate-950 text-slate-100 overflow-x-hidden">
      ...
      >
        {/* glow roxo — mais intenso */}
        <div
          className="absolute w-[900px] h-[900px] rounded-full -top-40 left-1/2 -translate-x-1/2 pointer-events-none"
          style={{ background: "radial-gradient(circle, rgba(139,92,246,0.28) 0%, transparent 60%)" }}
        />
        {/* glow azul */}
        <div
          className="absolute w-[500px] h-[500px] rounded-full bottom-0 right-[5%] pointer-events-none"
        ...
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-violet-400/40 bg-violet-400/[0.12] text-[0.72rem] font-semibold tracking-[0.12em] uppercase text-violet-300 mb-7">
          ...
          Plataforma de Avaliação Técnica
        </div>
        ...
        <p className="max-w-[520px] mx-auto mb-12 text-base leading-relaxed text-slate-300 font-light">
          Avalie projetos com métricas técnicas, gere classificações automáticas
          e acompanhe indicadores estratégicos em um dashboard interativo.
        </p>
      ...
      {/* ══ STATS STRIP — mais claro ══ */}
      <div className="flex justify-center border-t border-b border-slate-700/70 bg-slate-900/70 overflow-hidden">
        {stats.map((s, i) => (
          <div
            key={s.label}
            className={`flex-1 max-w-[240px] px-8 py-8 text-center transition-colors duration-300 hover:bg-violet-500/[0.08] ${
              i < stats.length - 1 ? "border-r border-slate-700/70" : ""
            }`}
          >
      ...
      <div className="max-w-[1200px] mx-auto px-12 py-[100px]">
        ...
        <h2 className="font-extrabold text-[clamp(1.8rem,3.5vw,2.8rem)] tracking-[-0.025em] leading-[1.15] mb-4">
          Três passos para<br />
          <span className="text-sky-400">decisões mais inteligentes</span>
        </h2>
 
        <p className="text-[0.95rem] text-slate-400 leading-relaxed max-w-[440px] mb-16">
          Fluxo simples, estratégico e orientado a dados para elevar
          o padrão de qualidade dos seus projetos.
        </p>
            ...
            {steps.map((s) => (
              ...
              >
                ...
                <div className="relative z-10 flex-1">
                  ...
                  <p className="text-[0.83rem] leading-relaxed text-slate-400">
                    {s.description}
                  </p>
                </div>
              </div>
      ...
      <div className="relative mx-12 mb-20 rounded-[28px] overflow-hidden border border-slate-700/50">
        ...
        {/* glow topo */}
        <div
          className="absolute -top-28 left-1/2 -translate-x-1/2 w-[600px] h-[300px] rounded-full pointer-events-none"
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10 px-16 py-16">
          ...
          <div className="flex-1">
            ...
            <p className="text-[0.9rem] text-slate-400 leading-relaxed max-w-[380px]">
              Crie sua conta gratuitamente e comece a avaliar
              a qualidade técnica dos seus projetos hoje mesmo.
            </p>
          </div>
 
          {/* lado direito — card com checklist + CTA */}
          <div className="flex-shrink-0 w-full md:w-[300px]">
            <div className="flex flex-col gap-5 p-7 rounded-2xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-sm">
              ...
              <Link
                href="/auth/login"
                className="text-center text-[0.8rem] text-slate-500 hover:text-slate-300 transition-colors"
              >
                Já tenho conta
              </Link>
            </div>
