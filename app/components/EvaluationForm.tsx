...
export type EvaluationFormData = {
  ...
  analyzedBy: string; description: string;
};
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-3xl border border-white/[0.07] bg-gradient-to-b from-slate-900/80 to-slate-950/90 backdrop-blur-xl shadow-2xl p-8 space-y-9">
      ...
      <div>
        ...
        <textarea placeholder={t("evaluationForm.descriptionPlaceholder")} value={description}
          onChange={(e) => setDescription(e.target.value)} rows={4} className={`${inputCls} resize-none`} />
        <p className="text-[0.72rem] text-slate-600 mt-1.5">{t("evaluationForm.descriptionHint")}</p>
      </div>
