...
const inputCls =
  "w-full rounded-xl px-3.5 py-2.5 bg-white/[0.03] border border-white/[0.07] text-slate-100 text-sm placeholder:text-slate-600 outline-none transition-all duration-200 focus:border-sky-500/60 focus:bg-sky-500/[0.04] focus:ring-2 focus:ring-sky-500/15";
 
const labelCls =
  "block text-[0.68rem] font-semibold tracking-[0.1em] uppercase text-slate-500 mb-1.5";
...
export default function EvaluationFilters({
  ...
  filterMaxScore, setFilterMaxScore,
    ...
    <div className="rounded-2xl border border-sky-500/15 bg-slate-900/50 backdrop-blur-sm overflow-hidden">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {(filterName || filterLanguage || filterClassification || filterMinScore !== "" || filterMaxScore !== "" || filterStartDate || filterEndDate) && (
            <span className="w-2 h-2 rounded-full bg-sky-400 animate-pulse" />
          )}
          {open
            ? <ChevronUp size={15} className="text-sky-400" />
      ...
      <div className={`transition-all duration-400 ease-in-out overflow-hidden ${open ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0"}`}>
        <div className="px-6 pb-6 border-t border-white/[0.05]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pt-5">
            ...
            {/* linguagem */}
            <div>
              <label className={labelCls}>Linguagem</label>
              <select
            ...
            {/* score máx */}
            <div>
              <label className={labelCls}>Score Máximo</label>
              <input
                type="number" min={0} max={100}
                placeholder="100"
                value={filterMaxScore}
                onChange={(e) => setFilterMaxScore(e.target.value === "" ? "" : Number(e.target.value))}
                className={inputCls}
              />
            </div>
          ...
          {/* classificação rápida por badge */}
          <div className="flex flex-wrap gap-2 mt-5">
            ...
            {(["EXCELENTE","BOM","REGULAR","RUIM"] as Classification[]).map((c) => (
              <button
                key={c}
                type="button"
                onClick={() => setFilterClassification(filterClassification === c ? "" : c)}
                className={`px-3 py-1 rounded-full text-[0.7rem] font-bold border transition-all duration-200 ${
                  filterClassification === c
                    ? classColors[c]
                    : "text-slate-600 border-slate-800 bg-transparent hover:border-slate-700 hover:text-slate-400"
                }`}
              >
                {c}
              </button>
          ...
          <div className="flex justify-end gap-3 mt-5 pt-4 border-t border-white/[0.05]">
            <button
              type="button"
              onClick={() => {
                setFilterName(""); setFilterLanguage(""); setFilterClassification("");
                setFilterMinScore(""); setFilterMaxScore(""); setFilterStartDate(""); setFilterEndDate("");
              }}
