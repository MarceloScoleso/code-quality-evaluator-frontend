...
export default function RegisterPage() {
  ...
  const passwordRules = [
    { id: "length",    label: t("register.passwordReqs.length"),   test: (v: string) => v.length >= 8 },
    { id: "uppercase", label: t("register.passwordReqs.uppercase"),test: (v: string) => /[A-Z]/.test(v) },
    { id: "number",    label: t("register.passwordReqs.number"),   test: (v: string) => /[0-9]/.test(v) },
  ];

  const registerSteps = t("register.steps", { returnObjects: true }) as { label: string; desc: string }[];
  ...
  const strength    = rulesPassed.length;
  ...
  const strengthBar   = ["", "w-1/3 bg-red-500", "w-2/3 bg-yellow-400", "w-full bg-green-500"];

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSubmitError("");
    if (!allValid) { setSubmitError(t("register.errorInvalid")); return; }
    setLoading(true);
    try {
      ...
      if (!res.ok) {
        ...
        setSubmitError(body?.message ?? t("register.errorServer"));
        return;
      }
      router.push("/auth/login");
    } catch {
      setSubmitError(t("register.errorConnect"));
    } finally {
      setLoading(false);
    }
  }
      ...
      <aside className="hidden lg:flex relative w-[52%] flex-shrink-0 flex-col justify-between px-14 py-12 overflow-hidden bg-slate-950">
        ...
        <div className="relative z-10 flex flex-col justify-center flex-1 py-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-green-400/30 bg-green-400/[0.06] w-fit mb-7">
            ...
            <span className="text-[0.65rem] font-semibold tracking-[0.14em] uppercase text-green-400">
              {t("register.eyebrow")}
            </span>
          </div>
        ...
        <p className="relative z-10 text-xs text-slate-600 tracking-wide">
          © {new Date().getFullYear()} CodeQuality — {t("register.footer")}
        </p>
      </aside>
          ...
          <div className="mb-8">
            <h2 className="text-3xl font-extrabold tracking-tight leading-snug mb-2">
              ...
              <span className="bg-gradient-to-r from-violet-400 to-sky-300 bg-clip-text text-transparent">
                {t("register.formAccent")}
              </span>
            </h2>
            <p className="text-sm text-slate-400 font-light">{t("register.formSubtitle")}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input type="text" placeholder={t("register.namePlaceholder")} value={name}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input type="email" placeholder={t("register.emailPlaceholder")} value={email}
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full rounded-xl pl-9 pr-11 py-3 bg-white/[0.03] border text-slate-100 text-sm placeholder:text-slate-600 outline-none transition-all duration-200 focus:ring-2 focus:ring-violet-500/20
                    ${password.length > 0
                      ? allValid ? "border-green-500/50 focus:border-green-500 focus:bg-green-500/[0.04]"
              ...
              {(passwordFocused || password.length > 0) && (
                <div className="mt-2 p-3 rounded-xl border border-white/[0.06] bg-white/[0.02] space-y-1.5">
                  <p className="text-[0.63rem] font-semibold tracking-widest uppercase text-slate-600 mb-2">
                    {t("register.passwordReqs.title")}
                  </p>
                  {passwordRules.map((rule) => {
                    const ok = rule.test(password);
                    return (
                      <div key={rule.id} className="flex items-center gap-2">
                        {ok ? <CheckCircle size={12} className="text-green-400 flex-shrink-0" />
